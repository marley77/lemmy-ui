...
if (!process.env["LEMMY_UI_DISABLE_CSP"]) {
  server.use(function (_req, res, next) {
    res.setHeader(
      ...
      `default-src 'none'; connect-src *; img-src * data:; script-src 'self' https://grow.biglife.quest 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; form-action 'self'; base-uri 'self'`
    );
    next();
  });
}
