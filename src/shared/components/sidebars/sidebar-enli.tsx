import { Component } from "inferno";
import { NavLink } from "inferno-router";
import { GetSiteResponse } from "lemmy-js-client";
// import { i18n } from "../../i18next";
// import { docsUrl, joinLemmyUrl, repoUrl } from "../../utils";
// import { VERSION } from "../../version";
// import { Icon } from "../common/icon";

// interface FooterProps {
//   site: GetSiteResponse;
// }

//    render() {
//     return (

// // export class Sidebarenlightenment extends Component<Sidebar, any> {
// //   constructor(props: any, context: any) {
// //     super(props, context);
// //   }

// export default SidebarEnli;

// class SidebarEnli extends Component {
//   render() {

export class SidebarEnli extends Component {
  render() {
    return (
      <div class="card border-secondary mb-3">
        {/*  PASTE HERE  ================ */}

<iframe sandbox="allow-scripts allow-forms allow-same-origin" src="https://biglifeboost.quest/post/3" height="1180px" width="400px"></iframe>


{/*        <div class="inner">
          <div class="post-feed">
 
          </div>
        </div>*/}

        {/* END PASTE   ================ */}
      </div>
    );
  }
}
